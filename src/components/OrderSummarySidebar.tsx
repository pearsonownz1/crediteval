...
const OrderSummarySidebar: React.FC<OrderSummarySidebarProps> = ({ orderData }) => {
  ...
  const { type, pageCount, evaluationType, languageFrom, languageTo } = orderData.services;

  // Determine line items based on service type
  const getLineItems = () => {
    let items = [];
    if (type === 'translation') {
      items.push({ name: 'Certified Translation', quantity: `${pageCount || 1} page(s)`, price: (pageCount || 1) * 25 });
      items.push({ name: `Languages`, quantity: `${languageFrom} to ${languageTo}`, price: null });
    } else if (type === 'evaluation') {
      const evalPrice = evaluationType === 'document' ? 85 : 150;
      const evalName = evaluationType === 'document' ? 'Document-by-Document' : 'Course-by-Course';
      items.push({ name: `Credential Evaluation (${evalName})`, quantity: 1, price: evalPrice });
    } else if (type === 'expert') {
    ...
    if (urgencyMultiplier > 1) {
        ...
        items.push({ name: `Urgency (${urgency})`, quantity: null, price: surcharge });
    } else {
      ...
      <CardContent className="space-y-4">
        {lineItems.map((item, index) => (
          <div key={index} className="flex justify-between text-sm">
            <span>
              {item.name}
              {item.quantity && typeof item.quantity === 'string' && !item.price && <span className="text-muted-foreground ml-1">({item.quantity})</span>}
               {item.quantity && typeof item.quantity === 'number' && item.quantity > 1 && <span className="text-muted-foreground ml-1">x{item.quantity}</span>}
            </span>
        ...
        <div className="flex justify-between font-semibold">
          <span>Total</span>
          <span className="text-primary">${calculatedPrice.toFixed(2)}</span> {/* Added text-primary */}
        </div>
