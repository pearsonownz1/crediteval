...
const ServiceSelectionStep = ({ data, updateData }: ServiceSelectionStepProps) => {
  return (
    <div className="space-y-6">
      ...
      {data.type === "translation" && (
        <>
          <div className="space-y-2">
            ...
            <Select value={data.languageFrom} onValueChange={(value) => updateData({ languageFrom: value })}>
          ...
          <div className="space-y-2">
            ...
            <Select value={data.languageTo} onValueChange={(value) => updateData({ languageTo: value })}>
            ...
            >
              <div className="flex items-start space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={() => updateData({ evaluationType: 'course' })}
                />
                <div>
                  <Tooltip>
                    <TooltipTrigger> {/* Removed asChild */}
                      <Label htmlFor="course-by-course" className="font-medium cursor-pointer underline decoration-dashed decoration-muted-foreground">
                        Course-by-Course ($150)
                      </Label>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p className="max-w-xs">Lists all courses with U.S. semester credits and grade equivalents. Recommended for further education or professional licensing.</p>
                    </TooltipContent>
                  </Tooltip>
                  <p className="text-sm text-muted-foreground">Detailed evaluation of all courses, grades, and degrees</p>
                </div>
              </div>
            </div>
            <div
            ...
            >
              <div className="flex items-start space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={() => updateData({ evaluationType: 'document' })}
                />
                <div>
                  <Tooltip>
                    <TooltipTrigger> {/* Removed asChild */}
                      <Label htmlFor="document-by-document" className="font-medium cursor-pointer underline decoration-dashed decoration-muted-foreground">
                        Document-by-Document ($85)
                      </Label>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p className="max-w-xs">Identifies your credentials and provides their U.S. equivalents. Suitable for employment or immigration purposes.</p>
                    </TooltipContent>
                  </Tooltip>
                  <p className="text-sm text-muted-foreground">Basic evaluation of degrees and diplomas only</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {data.type === "expert" && (
        <div className="space-y-2">
          ...
          <Select value={data.visaType} onValueChange={(value) => updateData({ visaType: value })}>
      ...
      <div className="space-y-2">
        ...
        <Select value={data.urgency} onValueChange={(value) => updateData({ urgency: value })}>
          ...
          <SelectContent>
            ...
            <SelectItem value="expedited">Expedited (3-5 business days)</SelectItem>
...
const DocumentUploadStep = ({
  ...
  const uploadFile = async (file: File, id: string) => {
    if (!orderId) {
      ...
      updateDocumentStatusById(id, { status: 'error', error: 'Order ID missing.' });
      return;
    }
    ...
    try {
      ...
      updateDocumentStatusById(id, { status: 'success', path: newPath, progress: 100 });
      ...
      try {
        ...
        const functionUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/update-order-documents`;
      ...
      updateDocumentStatusById(id, { status: 'error', error: uploadError.message || 'Upload failed', progress: undefined });
    }
  };
  ...
  return (
    ...
    <TooltipProvider>
      <div className="space-y-8">
        ...
        <ServiceSelectionStep data={serviceData} updateData={updateServiceData} />
      ...
      <div className="space-y-4">
        <div className="bg-muted p-4 rounded-lg space-y-3">
          <div>
            ...
            {orderData.customerInfo.phone && <p className="text-sm">{orderData.customerInfo.phone}</p>} {/* Display phone if available */}
          </div>
          ...
          <div>
            ...
            {orderData.services.type === "translation" && <p className="text-sm text-muted-foreground mt-1">You'll receive a certified translation suitable for official use.</p>}
            {orderData.services.type === "evaluation" && <p className="text-sm text-muted-foreground mt-1">You'll receive a report detailing the U.S. equivalency of your credentials.</p>}
            {orderData.services.type === "expert" && <p className="text-sm text-muted-foreground mt-1">You'll receive a letter analyzing your qualifications for your specific visa needs.</p>}
...
const OrderWizard = ({
  ...
  const totalSteps = 4; // Your Info, Service & Docs, Review, Payment
  ...
  const handleNext = async () => {
    ...
    console.log(`[DEBUG] handleNext start - Current orderId: ${orderId} (Type: ${typeof orderId})`); // Log orderId at the start of each call
      ...
      try {
        ...
        if (insertError) {
         ...
         if (data && data.id) {
            ...
            console.log(`[DEBUG] orderId state set in Step 0 to: ${orderIdString} (Type: ${typeof orderIdString})`); // Log string ID and type

            // --- Klaviyo Web SDK Tracking ---
            try {
              // Ensure _learnq is available (loaded by the script in index.html)
              if (window._learnq) {
                console.log("Klaviyo SDK (_learnq) found. Identifying user and tracking event.");
                ...
                const klaviyoApiKey = import.meta.env.VITE_KLAVIYO_PRIVATE_API_KEY; // Use environment variable
                ...
                if (klaviyoApiKey) {
                  ...
                  const functionUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-klaviyo-event`;
                  console.log(`Calling Supabase function for Klaviyo event: ${functionUrl}`);
                  ...
                  .then(async res => { // Make async to parse JSON body on error
                    if (!res.ok) {
                      // Attempt to parse error details from the Supabase function response
                      const errorData = await res.json().catch(() => ({ error: 'Failed to parse error response from Supabase function.' }));
                      console.error('Supabase function error sending Klaviyo event:', res.status, errorData);
                      ...
                      // throw new Error(`Supabase function request failed with status ${res.status}`);
                       ...
                       console.log('Supabase function successfully processed Klaviyo event (Status 204 No Content).');
                       // No body expected on 204 success
                    } else {
                       // Handle unexpected success statuses if necessary
                       const responseData = await res.json().catch(() => null);
                       console.log('Supabase function response (checkout_started):', responseData);
                    }
                  ...
                  .catch(err => console.error('Error calling Supabase function for Klaviyo event:', err));

                  console.log("Klaviyo identify call pushed (Web SDK) and checkout_started event sent via Supabase function.");

                } else {
                   console.warn("VITE_KLAVIYO_PRIVATE_API_KEY not found in environment variables. Klaviyo server-side event tracking skipped.");
                }

              } else {
                console.warn("Klaviyo SDK (_learnq) not found on window. Identify call skipped.");
              }
            } catch (klaviyoError) {
              console.error("Error during Klaviyo SDK interaction:", klaviyoError);
            }
    ...
    // --- End Save Customer Info ---
      ...
      console.log(`[DEBUG] At start of payment step (step 3) - orderId: ${orderId} (Type: ${typeof orderId})`); // Log orderId at start of payment step
      ...
      try {
        ...
        console.log(`[DEBUG] Preparing to call create-payment-intent. Order ID: ${orderId}, Type: ${typeof orderId}`);
        if (!orderId || typeof orderId !== 'string' || orderId.trim() === '') { // Check type and for empty string
          console.error("[CRITICAL] orderId is invalid before calling create-payment-intent function!", { orderId });
          // Throw an error here to prevent the fetch call if orderId is invalid
           throw new Error("Frontend Error: Invalid Order ID before calling payment function. Please go back and try again.");
        }
       ...
       if (currentStep === totalSteps - 1) { // Check if on the *new* final step (index 3)
           setError("Stripe is not ready. Please wait a moment and try again.");
       } else if (currentStep < totalSteps -1) { // If not on the final step, just move forward
  ...
  const steps = [
    ...
    { title: "Service & Docs", icon: <Globe className="h-5 w-5" /> }, // Combined Step
  ...
  return (
    ...
    <div className="container mx-auto py-12 px-4 grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
      ...
      <div className="md:col-span-2">
        <Card> {/* Removed className="md:col-span-2" from Card */}
          <CardHeader>
        ...
        <CardContent>
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {currentStep === 2 && <ReviewStep orderData={orderData} />} {/* Review is now step 2 */}

              {currentStep === 3 && ( /* Payment is now step 3 */
        ...
        <CardFooter className="flex justify-between">
          ...
          <Button onClick={handleNext} disabled={
            ...
            {isSubmitting && currentStep === 0 ? "Saving..." :
             ...
             currentStep === totalSteps - 1 ? "Complete Order" : "Next"} {/* Check against new totalSteps */}
            {!(isSubmitting && currentStep === 0) && !paymentProcessing && currentStep !== totalSteps - 1 && ( // Check against new totalSteps
              <ArrowRight className="ml-2 h-4 w-4" />
            )}
          </Button>
        </CardFooter> {/* Closing CardFooter */}
       </Card> {/* Closing Card */}
      </div> {/* Closing Main Content Area div */}

      {/* Sidebar Area - takes 1 column */}
      <div className="md:col-span-1 space-y-6"> {/* Removed mt-8 md:mt-0 */}
        {/* Order Summary Sidebar */}
        <OrderSummarySidebar orderData={orderData} />

        {/* Social Proof Section (Below Sidebar) */}
        <div className="mt-8"> {/* Add margin top to separate from sidebar */}
          <h3 className="text-xl font-semibold text-center md:text-left">Why Choose CreditEval?</h3>
          {socialProofItems.map((item, index) => (
            <div key={index} className="flex items-start gap-4 p-4 bg-gray-50 rounded-lg mt-4"> {/* Added mt-4 */}
        ...
        </div> {/* Closing Social Proof Items container */}

        {/* Added Trust Logos */}
        <div className="mt-6 pt-6 border-t border-gray-200">
          <h4 className="text-center text-sm font-medium text-muted-foreground mb-4">Trusted & Accredited</h4>
          <div className="flex justify-center items-center space-x-4">
            <img src="/ata-logo.png" alt="ATA Member" className="h-10" />
            <img src="/bbb-accredited.png" alt="BBB Accredited Business" className="h-10" />
            <img src="/trustpilot-logo.png" alt="Trustpilot" className="h-8" />
          </div>
        </div>
        {/* End Added Trust Logos */}
      </div> {/* Closing Sidebar Area div */}
