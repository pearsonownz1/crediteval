...
const stripe = stripeSecretKey ? new Stripe(stripeSecretKey, { apiVersion: "2023-10-16", httpClient: Stripe.createFetchHttpClient() }) : null;
...
serve(async (req) => {
  ...
  if (!stripe || !supabaseAdmin || !stripeWebhookSigningSecret) {
    console.error("Missing critical environment variables (Stripe, Supabase, or Webhook Secret).");
    return new Response(JSON.stringify({ error: "Server configuration error." }), { status: 500 });
  }
  // PostHog key is needed for event tracking but not critical for webhook function itself
  if (!posthogApiKey) {
    console.warn("Missing POSTHOG_API_KEY environment variable. Payment completion events will not be tracked.");
  }
  ...
  try {
    ...
    console.log(`Webhook event received: ${receivedEvent.id}, Type: ${receivedEvent.type}`);
  } catch (err) {
  ...
  if (receivedEvent.type === "checkout.session.completed") {
    ...
    if (paymentStatus === "paid" && quoteId) {
      console.log(`Payment successful for Quote ID: ${quoteId}. Processing order creation and status update.`);
      try {
        ...
        if (fetchError || !quoteData) {
          console.error(`Failed to fetch quote ${quoteId} for order creation:`, fetchError);
        ...
        const { data: orderInsertResult, error: orderInsertError } = await supabaseAdmin
          .from("orders")
          .insert(newOrderData)
        ...
        if (orderInsertError) {
          console.error(`Failed to insert order for quote ${quoteId}:`, orderInsertError);
          throw new Error(`Database error creating order from quote.`);
        }

        const newOrderId = orderInsertResult?.id;
        console.log(`Successfully created order ${newOrderId} from quote ${quoteId}.`);
        ...
        if (quoteUpdateError) {
          console.error(`Failed to update quote ${quoteId} status to Paid:`, quoteUpdateError);
        ...
        if (posthogApiKey && quoteData.email) {
          ...
          console.log(`Sending payment_completed event to PostHog for quote ${quoteId}`);
          ...
          .then(response => {
            if (!response.ok) {
              console.error(`PostHog API request failed with status ${response.status}:`, response.statusText);
              response.text().then(text => console.error("PostHog API response body:", text));
            } else {
              console.log(`PostHog event payment_completed sent successfully for quote ${quoteId}.`);
            }
          })
          .catch(err => {
        ...
        if (newOrderId && RESEND_API_KEY && FROM_EMAIL) {
            ...
            const staffNotificationBody = `
              ...
              <ul>
                ...
                <li><strong>Service Type:</strong> ${quoteData.service_type}</li>
                <li><strong>Amount Paid:</strong> $${(orderTotalAmount / 100).toFixed(2)}</li>
              </ul>
            ...
            try {
                console.log(`Sending staff notification for order ${newOrderId} to: ${staffEmails.join(', ')}`);
                ...
                if (!staffResendResponse.ok) {
                    ...
                    console.error('Failed to send staff notification email:', staffResendData);
                } else {
                    console.log(`Staff notification email sent successfully for order ${newOrderId}.`);
                }
            } catch (emailError) {
                 console.error('Exception sending staff notification email:', emailError);
            }
        } else {
             console.warn(`Skipping staff notification for order ${newOrderId} due to missing config or order ID.`);
        }

      } catch (processingError) {
        console.error(`Error processing paid quote ${quoteId}:`, processingError);
        // Return 500 to signal Stripe to retry if processing failed critically
        return new Response(JSON.stringify({ error: "Webhook handler failed", details: processingError.message }), { status: 500 });
      }
    } else {
      console.log(`Ignoring checkout session ${session.id}. Payment status: ${paymentStatus}, Quote ID: ${quoteId}`);
    }
  } else {
