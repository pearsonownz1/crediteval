...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Processing request to add document path '${documentPath}' to order ${orderId}`
    );
    ...
    if (fetchError) {
      console.error(
        `Error fetching order ${orderId} for document update:`,
        fetchError
      );
    ...
    const updatedPaths = currentPaths.includes(documentPath)
      ? currentPaths
      : [...currentPaths, documentPath];
    ...
    if (updateError) {
      console.error(
        `Error updating document_paths for order ${orderId}:`,
        updateError
      );
      throw new Error(
        `Failed to update order documents: ${updateError.message}`
      );
    }
    // --- End Update ---

    console.log(
      `Successfully added document path '${documentPath}' to order ${orderId}.`
    );

    return new Response(
      JSON.stringify({
        success: true,
        message: "Document path added successfully.",
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
